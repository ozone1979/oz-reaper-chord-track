param(
    [string]$DestinationRoot = (Join-Path $env:APPDATA "REAPER\Scripts\Oz Reaper Chord Track\Scripts"),
    [string]$SourceRoot,
    [switch]$Verify
)
...
$sourceCore = Join-Path $SourceRoot "Oz Chord Track Core.lua"
...
Copy-Item -LiteralPath $sourceCore -Destination (Join-Path $DestinationRoot "Oz Chord Track Core.lua") -Force
