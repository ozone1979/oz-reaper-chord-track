...
Replace `X.Y.Z` with the next version (for example `0.1.2`):

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\publish-reapack-release.ps1 -Version "X.Y.Z" -GithubOwner "ozone1979" -RepoName "oz-reaper-chord-track" -Author "ozone1979"
```
...
- Release exists:
  - `gh release view vX.Y.Z --repo ozone1979/oz-reaper-chord-track`
- Feed has the new version and only tag-pinned URLs:

```powershell
$u='https://raw.githubusercontent.com/ozone1979/oz-reaper-chord-track/main/index.xml'
...
## 4) ReaPack client check

In REAPER/ReaPack:

1. `Extensions` → `ReaPack` → `Synchronize packages`
2. Confirm package `Oz Reaper Chord Track` shows update `X.Y.Z`
3. Install/update and quick-smoke one or two key actions
